...
<p align="center">
  <strong>The eBay MCP server — give Claude, Cursor, and any AI assistant full access to eBay's Sell APIs. 297 tools for inventory, orders, marketing, and analytics, running locally with your own keys.</strong>
</p>
...
<p align="center">
  <img src="https://img.shields.io/badge/tools-297-8957e5?logo=ebay&logoColor=white" alt="297 eBay API tools" />
...
- **297 eBay API tools** — 100% coverage of the eBay Sell APIs across inventory, orders, marketing, analytics, metadata, taxonomy, and developer tooling.
...
| `developer` / `token-management` | API status, OAuth URLs, token refresh, and diagnostics | "Check whether eBay APIs and my token are healthy." |
...
| API coverage | 297 tools across 100% of the Sell APIs, ready to call | Build each request from the docs |
...
That should call `ebay_developer_analytics_get_rate_limits` or `ebay_developer_analytics_get_user_rate_limits` and confirms the server, credentials, and MCP client wiring without changing seller data.
...
<details open>
<summary><strong>297 tools by category (100% Sell API coverage)</strong></summary>

**297 tools**, 100% Sell API coverage, organized by category. Each link points to the module that owns its definitions and handlers:
...
A local [Model Context Protocol](https://modelcontextprotocol.io) server that exposes **297 tools** covering **100% of eBay's Sell APIs** (270 endpoints) to AI assistants — inventory, order fulfillment, marketing, analytics, and developer tools.
...
297 tools across 270 unique endpoints — 100% of eBay's Sell APIs.
