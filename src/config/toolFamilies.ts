...
const parseFamilyTokens = (raw: string): string[] =>
  raw
    .split(',')
 ...
 *
 * @param env - Environment object containing optional `EBAY_MCP_TOOLS`.
 * @returns Tool gating mode parsed from the environment.
 *
 * @example
 * ```ts
 * const mode = resolveToolGatingMode({ EBAY_MCP_TOOLS: 'inventory,fulfillment' });
 * ```
 */
export const resolveToolGatingMode = (env: NodeJS.ProcessEnv = process.env): ToolGatingMode => {
 ...
 * @example
 * ```ts
 * const error = getToolGatingConfigError({ EBAY_MCP_TOOLS: 'inventory' });
 * ```
 */
