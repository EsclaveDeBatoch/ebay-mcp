  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        '1. One-time account setup: `ebay_sell_account_opt_in_to_program` (business-policy management), then `ebay_sell_account_create_payment_policy`, `ebay_sell_account_create_return_policy`, `ebay_sell_account_create_fulfillment_policy` for reusable policy IDs, and `ebay_create_inventory_location` for a `merchantLocationKey`.',
        ...
        '',
        '**Fulfill an order.**',
        '`ebay_sell_fulfillment_get_orders` (filter unfulfilled) → `ebay_sell_fulfillment_get_order` (detail) → `ebay_sell_fulfillment_create_shipping_fulfillment` (tracking number + carrier).',
        '',
        '**Refund or handle a payment dispute.**',
        'Voluntary refund: `ebay_sell_fulfillment_issue_refund`. Disputes: `ebay_sell_fulfillment_get_payment_dispute_summaries` → `ebay_sell_fulfillment_get_payment_dispute` → `ebay_sell_fulfillment_accept_payment_dispute` (or contest it).',
        '',
        ...
        '',
        '**Inspect the authenticated eBay profile.**',
...
export const buildContributingDoc = (snapshot: RegistrySnapshot): SkillDoc => ({
  ...
  description: `Work on the ebay-mcp server itself: ${snapshot.toolCount} tools across eBay's Sell APIs (TypeScript/ESM, direct Zod 4 resource contracts, OpenAPI-generated types). Use when migrating or changing eBay resources, wiring the explicit catalogue, or running the repository checks.`,
  intro: `A local MCP server exposing **${snapshot.toolCount} tools across 100% of eBay's Sell APIs** — TypeScript/Node.js (ESM), \`@modelcontextprotocol/sdk\`, direct Zod 4 validation, native promises, and OpenAPI-generated types. Entry points: \`src/index.ts\` (STDIO) and \`src/serverHttp.ts\` (HTTP).`,
  sections: [
    {
      heading: 'Validation (run before a PR)',
      body: [
        '```bash',
        'pnpm run check:ci        # Biome plus the code-style contract',
        'pnpm run typecheck       # server source',
        'pnpm run typecheck:ui    # browser source',
        'pnpm run typecheck:tests # test source',
    ...
    },
    {
      ...
      body: [
        ...
        '| `config/` | Process-entry environment contracts and tool exposure |',
        '| `ebay/` | Seller session, typed completion, and official resource modules |',
        '| `mcp/` | Strict tool boundary, explicit catalogue, runtime, and exposure |',
        '| `ui/` | Browser views and presentation-only eBay document projections |',
        "| `skills/` | Agent-skills generator (this skill's source) |",
        '| `generated/ebay/` | Machine-owned TypeScript generated from `specs/ebay/` |',
        '| `api/`, `schemas/`, `tools/`, `types/`, `utils/`, `scripts/` | Legacy migration inventory; do not extend these abstractions |',
      ].join('\n'),
    },
    {
      heading: 'Add a tool or endpoint',
      body: [
        '1. Run `npm run sync`, then locate the official namespace, API, resource, operation, and generated response type.',
        '2. Write failing colocated resource tests and real-MCP integration tests for strict fields, exact wire behavior, document passthrough, and failures.',
        '3. Create or extend `src/ebay/<namespace>/<api>/<resource>.ts` with one strict Zod arguments schema and a named promise operation.',
        '4. Return `EbayRequestCompletion<GeneratedEbayDocument>` unchanged and define the hierarchical MCP tool beside the operation.',
        '5. Import the named definition explicitly in `src/mcp/ebayToolCatalogue.ts`; do not add a barrel, array, handler adapter, or compatibility alias.',
        '6. Delete the old API/schema/category path when its final caller moves.',
        '7. Run the complete validation gate and hermetic integration suite.',
      ].join('\n'),
    },
    {
      heading: 'Conventions',
      body: [
        '- Use exact eBay wire keys in one strict direct Zod 4 boundary; never add new Effect or schema-adapter code.',
        '- Export named arrow functions, use precise domain identifiers, and preserve generated eBay documents without reshaping.',
        '- Do not add compatibility exports, old tool-name aliases, passive barrels, facade methods, or one-use wrappers.',
        '- Follow `CODE-STYLE.md` for the forbidden identifier, control-flow, structure, UI, CLI, and dependency rules.',
        '- Commit with Conventional Commits. Releases are tag-driven; PR labels select major or minor bumps.',
