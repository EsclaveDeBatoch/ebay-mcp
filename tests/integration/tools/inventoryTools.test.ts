...
describe('Inventory Tools Integration Tests', () => {
  ...
  describe('ebay_get_inventory_items', () => {
    it('retrieve inventory items successfully', async () => {
      const mockResponse = {
        total: 2,
        limit: 25,
        inventoryItems: [
  ...
  describe('ebay_get_inventory_item', () => {
    it('retrieve specific inventory item by SKU', async () => {
      const mockItem = {
        sku: 'TEST-SKU-001',
        availability: {
  ...
  describe('ebay_create_or_replace_inventory_item', () => {
    it('create new inventory item successfully', async () => {
      const inventoryItem = {
        availability: {
          shipToLocationAvailability: {
            quantity: 50,
      ...
      await executeTool(api, 'ebay_create_or_replace_inventory_item', {
        sku: 'NEW-SKU-001',
        body: inventoryItem,
      });
    ...
    it('handle validation errors', async () => {
      ...
      await expect(
        executeTool(api, 'ebay_create_or_replace_inventory_item', {
          sku: 'NEW-SKU-001',
          body: invalidItem,
        }),
      ).rejects.toThrow();
  ...
  describe('ebay_get_inventory_locations', () => {
    it('retrieve inventory locations', async () => {
      const mockResponse = {
        total: 1,
        locations: [
          {
  ...
  describe('ebay_create_inventory_location', () => {
    it('create inventory location successfully', async () => {
      const locationData = {
        location: {
          address: {
            addressLine1: '123 Main St',
      ...
      await executeTool(api, 'ebay_create_inventory_location', {
  ...
  describe('Tool Parameter Validation', () => {
    it('throw error when SKU is missing for ebay_get_inventory_item', async () => {
