...
<p align="center">
  <strong>O servidor MCP do eBay — dê ao Claude, ao Cursor e a qualquer assistente de IA acesso completo às Sell APIs do eBay. 290 ferramentas para estoque, pedidos, marketing e analytics, rodando localmente com suas próprias chaves.</strong>
</p>
...
<p align="center">
  <img src="https://img.shields.io/badge/tools-290-8957e5?logo=ebay&logoColor=white" alt="290 ferramentas da API do eBay" />
...
**eBay MCP** é um servidor local do [Model Context Protocol](https://modelcontextprotocol.io) que conecta assistentes de IA —Claude Desktop, Claude Code, Cursor, Cline, Windsurf, Zed, Continue.dev, Roo Code e Amazon Q— diretamente às **Sell APIs do eBay**. Ele expõe **290 ferramentas** abrangendo **100% da superfície da Sell API do eBay** (266 endpoints únicos) para gestão de estoque, processamento de pedidos, marketing de anúncios patrocinados, analytics e ferramentas para desenvolvedores. Tudo roda na sua máquina via STDIO ou HTTP local — **sem relay na nuvem**, e suas credenciais do eBay nunca saem do seu computador.
...
- **290 ferramentas da API do eBay** — 100% de cobertura das Sell APIs do eBay em estoque, pedidos, marketing, analytics, metadados, taxonomia e ferramentas para desenvolvedores.
...
| Cobertura da API | 290 ferramentas em 100% das Sell APIs, prontas para usar | Você constrói cada requisição a partir da documentação |
...
Autoconfigurados pelo `npm run setup`. Requer Node.js ≥ 22.12 e o protocolo MCP 1.0+ via STDIO (padrão) ou HTTP.
...
**290 ferramentas**, 100% de cobertura da Sell API, organizadas por categoria. Cada link aponta para as definições de ferramentas e seus handlers em [`src/tools/categories/`](src/tools/categories/):
...
| [Account](src/tools/categories/account.ts) | Status obsoleto do programa de pagamentos; imposto sobre vendas |
| [Sell Account](src/ebay/sell/account/) | Status do vendedor, adesão a programas, assinaturas, tabelas de tarifas, elegibilidade para publicidade e políticas comerciais |
...
| [Metadata](src/ebay/sell/metadata/) | Políticas de devolução, jurisdições de imposto sobre vendas, compatibilidade automotiva |
...
**Ferramentas de exemplo:** `ebay_get_inventory_items`, `ebay_sell_fulfillment_get_orders`, `ebay_create_offer`, `ebay_get_campaigns`, `ebay_get_oauth_url`.
...
Um servidor local do [Model Context Protocol](https://modelcontextprotocol.io) que expõe **290 ferramentas** cobrindo **100% das Sell APIs do eBay** (266 endpoints) para assistentes de IA — estoque, processamento de pedidos, marketing, analytics e ferramentas para desenvolvedores.
...
290 ferramentas em 266 endpoints únicos — 100% das Sell APIs do eBay.
...
Node.js ≥ 22.12, uma [conta de desenvolvedor do eBay](https://developer.ebay.com/) gratuita (Client ID + Client Secret) e então rodar `npm run setup`.
